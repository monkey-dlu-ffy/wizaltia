...
const validateLearning = [
  ...
  body('collegeName').trim().optional().escape(),
  body('email').trim().isEmail().withMessage('A valid email address is required.').normalizeEmail(),
  body('phone').trim().notEmpty().withMessage('Phone number is required.').escape(),
  body('course').trim().notEmpty().withMessage('Please select a course.').escape(),
  body('message').trim().optional().isLength({ max: 2000 }).escape(),
];

/**
 * POST /api/learning
 * Learning Solutions enquiry form
 */
router.post('/', learningLimiter, validateLearning, async (req, res) => {
  ...
  const html = `
    ...
    <head>
      ...
      <style>
        ...
        .logo-box { width: 36px; height: 36px; background: #a855f7; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 16px; color: #fff; }
        ...
        .field.highlight p { color: #a855f7; }
      ...
      <div class="wrapper">
        <div class="header">
          <div class="header-brand">
            ...
            <div>
              <div class="brand-name">Wizaltia Learning Solutions</div>
              <div class="brand-sub">Industry-Focused Training Programs</div>
            </div>
          </div>
        ...
        <div class="body">
          ...
          <div class="field">
            ...
            <p>${collegeName || '(Not provided)'}</p>
          </div>
          <div class="field">
            <label>Email Address</label>
            <p><a href="mailto:${email}" style="color:#a855f7">${email}</a></p>
          </div>
          <div class="field">
            <label>Phone Number</label>
            <p><a href="tel:${phone}" style="color:#a855f7">${phone}</a></p>
          </div>
